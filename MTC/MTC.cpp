...
void MTC::calculateTimerSettings()
{
	...
	timerBits = ui.comboBox_timerBits->currentText().toInt();
...
void MTC::displayTimerSettingsText()
{
	...
	if (timerBits == 8)
	{
		TCCRA = "\tTCCR" + timerNoStr + "A = " + "_BV( WGM" + timerNoStr + "1 )" + "; \n";
	}
	else
	{
		TCCRB += "_BV( WGM" + timerNoStr + "2 ) | ";
	}

	TCCRB += CSn0 ? "_BV( CS" + timerNoStr + "0 )" : "";
	TCCRB += (CSn0 && CSn1) ? " | " : "";
	TCCRB += CSn1 ? "_BV( CS" + timerNoStr + "1 )" : "";
	TCCRB += ((CSn0 && CSn2) || (CSn1 && CSn2)) ? " | " : "";
	TCCRB += CSn2 ? "_BV( CS" + timerNoStr + "2 )" : "";
